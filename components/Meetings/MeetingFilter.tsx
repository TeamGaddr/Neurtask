...
const FilterButton: FC<FilterButtonProps> = ({ label, onClick }) => (
  <button
    onClick={onClick}
    className="flex items-center gap-1 px-3 py-1.5 sm:px-4 sm:py-2 border border-gray-300 rounded-2xl text-xs sm:text-sm text-gray-700 hover:bg-gray-100 transition bg-white"
  >
    {label}
...
const DateTimePickerModal: FC<{ onClose: () => void; onDateSelect: (date: Date) => void }> = ({
  onClose,
  onDateSelect,
}) => {
  ...
  <div key={`${d}-${idx}`} className="text-gray-500">
    {d}
  </div>
))}
...
const FilterModal: FC<{ title: string; options: string[]; onClose: () => void }> = ({
  title,
  options,
  onClose,
}) => (
...
const MeetingFilter: FC<MeetingFilterProps> = ({ onDateChange }) => {
  ...
  return (
    <div className="flex space-x-4 px-[10px] sm:px-[40px]">
      <div className="relative">
        <FilterButton label="Status" onClick={() => toggleFilter("Status")} />
        {activeFilter === "Status" && (
          <FilterModal
            title="Status"
      ...
      <div className="relative">
        <FilterButton label="Category" onClick={() => toggleFilter("Category")} />
        {activeFilter === "Category" && (
          <FilterModal
            title="Category"
            options={["Category Name 1", "Category Name 2"]}
            onClose={closeModal}
          />
        )}
      </div>
      ...
      <div className="relative">
        <FilterButton label="User" onClick={() => toggleFilter("User")} />
        {activeFilter === "User" && (
          <FilterModal
            title="User"
            options={["John", "Elisabeth", "Agent Name"]}
            onClose={closeModal}
          />
        )}
      </div>
      ...
      <div className="relative">
        <FilterButton label="Date" onClick={() => toggleFilter("Date")} />
        {activeFilter === "Date" && (
          <DateTimePickerModal
            onClose={closeModal}
